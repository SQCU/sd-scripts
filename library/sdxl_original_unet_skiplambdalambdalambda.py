...
class ResnetBlock2D(nn.Module):
    ...
    def forward_body(self, x, emb):
        ...
        if hasattr(self, "learnedlambda1"):
            x = self.skip_connection(x)*self.learnedlambda1
        else:
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        #self.learned_lambda_level = learned_lambda_level
        #if self.learnedlambda1 is None:
        #if learned_lambda_level == 1:   #cleaning up the network def
        #    self.learnedlambda1 = nn.Parameter(torch.tensor(0.85))
        #if learned_lambda_level == 3:   #i don't think == 2 is a coherent choice
