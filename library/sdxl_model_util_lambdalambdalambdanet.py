...
#from library import sdxl_original_unet_skiplambdalambda
from library import sdxl_original_unet_skiplambdalambdalambda
...
def _load_state_dict_on_device(model, state_dict, device, dtype=None):
    ...
    #slambda those lambdas!!! this better only trigger once ever in any geneaology of models!!!
    ...
    # reproducing the control flow for nonstandard, uh stuff. if you see this comment, good luck.    
    for k in list(state_dict.keys()):
        set_module_tensor_to_device(model, k, device, value=state_dict.pop(k), dtype=dtype)
...
def load_models_from_sdxl_checkpoint(model_version, ckpt_path, map_location, dtype=None, disable_mmap=False, initdict={}):
    ...
    with init_empty_weights():
        unet = sdxl_original_unet_skiplambdalambdalambda.SdxlUNet2DConditionModel(**initdict)
