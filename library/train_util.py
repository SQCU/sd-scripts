...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    ...
    parser.add_argument(
        ...
        type=str,
        default="snr",
        choices=["constant", "exponential", "snr", "vsignsnr", "shiftsignsnr"],
    ...
    parser.add_argument(
        "--khrapov_et_al_scheduled_huber_coefficient",
        action="store_true",
        default=False,
...
def get_timesteps_and_huber_c(args, min_timestep, max_timestep, noise_scheduler, b_size, device):
    ...
    # as. In the future there may be a smarter way
        ...
        # a quick review of how to apply a function elementwise along a torch tensor:
        #def flimmy(tenny, alpha):
        #   return  torch.exp(-alpha * tenny)
        #...
        #tensor_reference_symbol.apply_(flimmy)
        #however apply_ is forbidden for cuda operations, 
    ...
    def hubsched_vsigsnr(timestep):
        ...
        siglogsnr = torch.sigmoid(-torch.log(twise_snr)+1)+huber_epsilon  #kept getting 'huber delta must be positive', suggesting huber_c was being compressed to 0?  
        return siglogsnr*args.huber_c

    def hubsched_shiftsigsnr(timestep):
        alphas_cprd = noise_scheduler.alphas_cumprod[timestep].detach()
        alpha = alphas_cprd**0.5
        sigma = (1-alphas_cprd)**0.5
        twise_snr = (alpha/sigma)**2
        siglogsnr = torch.sigmoid(-torch.log(twise_snr))+huber_epsilon  #kept getting 'huber delta must be positive', suggesting huber_c was being compressed to 0?  
        return siglogsnr*args.huber_c+1    #minimum huber_c=1, maximum huber_c of args.huber_c+1. bc huber_c is arbitrary, it doesn't matter if the maximum value is offset.
    ...
    elif args.huber_schedule == "shiftsignsnr":
        for idx, t in enumerate(timesteps):
    ...
    #   actually we don't even need this block. why wouldn't constant be the default case? unbelievable.
    #else
    #    if args.loss_type == "huber" or args.loss_type == "smooth_l1":
    #        logger.info(f"how did you even trigger this codepath?")
...
def get_noise_noisy_latents_and_timesteps(args, noise_scheduler, latents):
    ...
    if args.khrapov_et_al_scheduled_huber_coefficient:
...
def conditional_loss(
    ...
    if loss_type == "l2":
        ...
        if huber_c.numel() == 1:
            loss = torch.nn.functional.huber_loss(model_pred, target, reduction=reduction, delta=huber_c.item())
        #if huber_c.size(0)>1:
        else:
        ...
        #please allow your users to apply loss weightings and other transforms along the batch dimension!
